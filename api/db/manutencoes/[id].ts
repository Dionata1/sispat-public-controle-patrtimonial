...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!id) {
    return res.status(400).json({ error: 'Identificador da manutenção é obrigatório.' });
  }
  ...
  try {
    const { isDbAvailable, deleteManutencaoDb } = await import('../../../src/db/dbService');
    if (!(await isDbAvailable())) {
      return res.status(503).json({ error: 'PostgreSQL indisponível no momento.' });
    }
    await deleteManutencaoDb(id);
