...
import { validateManutencaoPayload } from '../../../src/db/validation';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const { isDbAvailable, getAllManutencoesDb } = await import('../../../src/db/dbService');
  ...
  if (req.method === 'POST' || req.method === 'PUT') {
    ...
    try {
      const { isDbAvailable, upsertManutencaoDb } = await import('../../../src/db/dbService');
      if (!(await isDbAvailable())) {
        return res.status(503).json({ error: 'PostgreSQL indisponível no momento.' });
      }
