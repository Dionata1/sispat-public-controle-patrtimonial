...
import { validateAuditLogPayload } from '../../../src/db/validation';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const { isDbAvailable, getAllAuditLogsDb } = await import('../../../src/db/dbService');
  ...
  if (req.method === 'POST') {
    ...
    try {
      const { isDbAvailable, insertAuditLogDb } = await import('../../../src/db/dbService');
      if (!(await isDbAvailable())) return res.status(503).json({ error: 'PostgreSQL indisponível no momento.' });
