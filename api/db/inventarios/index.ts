...
import { validateInventarioPayload } from '../../../src/db/validation';
import type { InventarioSessao } from '../../../src/types';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const { isDbAvailable, getAllInventariosDb } = await import('../../../src/db/dbService');
  ...
  if (req.method === 'POST' || req.method === 'PUT') {
    ...
    try {
      const { isDbAvailable, upsertInventarioDb } = await import('../../../src/db/dbService');
      if (!(await isDbAvailable())) return res.status(503).json({ error: 'PostgreSQL indisponível no momento.' });
