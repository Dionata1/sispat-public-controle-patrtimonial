...
import { requireAuth } from '../../../src/db/requireAuth';
import { generateTemporaryPasswordNode, hashPasswordNode } from '../../../src/db/password';
import type { UserAccount, UserRole } from '../../../src/types';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const { isDbAvailable, listUsersDb } = await import('../../../src/db/dbService');
  ...
  if (req.method === 'POST') {
    try {
      const { isDbAvailable, listUsersDb, upsertUserDb } = await import('../../../src/db/dbService');
      if (!(await isDbAvailable())) {
        return res.status(503).json({ error: 'PostgreSQL indisponível no momento.' });
      }
      const all = await listUsersDb();
